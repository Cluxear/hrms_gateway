...
export class ApplicationService {
  public resourceUrl = SERVER_API_URL + 'services/applicationapp/api/applications';
  ...
  findByJobPost(id: number): Observable<EntityArrayResponseType> {
    return this.http
      .get<IApplication[]>(`${this.resourceUrl}/jobpost/${id}`, { observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient(application: IApplication): IApplication {
    const copy: IApplication = Object.assign({}, application, {
      creationDate: application.creationDate && application.creationDate.isValid() ? application.creationDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((application: IApplication) => {
        application.creationDate = application.creationDate ? moment(application.creationDate) : undefined;
      });
    }
    return res;
  }
